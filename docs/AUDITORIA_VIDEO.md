...
El MP4 regenerado con el núcleo, la captura y las mediciones finales pasó los 20 controles técnicos de `evidence/video_audit/verification.json`. La revisión separa exactitud de datos, calidad del plan visual y funcionamiento del archivo exportado. El informe y las imágenes de la versión anterior se conservaron en `before_final_guard/`. El estado inicial previo a exportación quedó guardado como `pending` en `pre_export_status.json`, sin aprobar controles antes de que existiera el video.
...
| VID-10 · P2 · Metadatos corruptos en la evidencia | El constructor leía la salida UTF-8 de FFprobe usando la codificación local de Windows. El informe de exportación guardaba «VÃ‰RTICE» y «decisiÃ³n», aunque el MP4 tenía los acentos correctos. | El constructor decodifica explícitamente UTF-8. Se comprobó que título y artista del informe coinciden con una lectura UTF-8 estricta del archivo real, y que los nueve capítulos conservan sus acentos. `probe_utf8_before_fix.json` y `probe_utf8_after_fix.json` identifican la observación anterior y la regresión. |
...
- La gráfica usa las siete variantes **con traza**, mediana de nueve repeticiones, y separa solucionador de JSON→JSON. Para el grafo dirigido de 500 nodos y 4000 conexiones, los datos finales guardados son 14.3257 ms y 124.3811 ms; la etiqueta visible de JSON redondea a 124.4 ms. Son resultados de CPython 3.11 en una computadora, sin garantía de latencia en navegador o vehículo. Las otras seis etiquetas también se contrastaron con sus medianas guardadas.
...
El propio verificador recibió revisión independiente. El primer criterio de error global podía aceptar un título o subtítulo borrado, porque esas letras ocupan una proporción pequeña del fotograma. Al evaluar por separado título y subtítulos, los 18 borrados deliberados sobre las nueve escenas produjeron errores regionales de 11.057–25.249 y 7.666–10.612, respectivamente; todos superan el límite 3. Esa prueba histórica se hizo en memoria, sin alterar el MP4, y se reproduce con `scripts/probe_film_pixels.py`; sus datos y hashes están en `verifier_image_mutations.json`. No se repitió en esta revisión porque el criterio de comparación no cambió. En los fotogramas comprimidos del nuevo MP4, los errores regionales máximos fueron 1.659 para título y 1.892 para subtítulos; el criterio aceptó la compresión real.

## Resultado del archivo final

- MP4 de 11,540,070 bytes; video de 209.966667 s y contenedor/audio de 210 s. La diferencia equivale aproximadamente a la duración de un fotograma de video.
...
- Se inspeccionaron visualmente el mosaico de dieciséis fotogramas y, a tamaño completo, traza, rendimiento, dirección inversa, foco del solucionador, nueva captura del estudio y último fotograma. Los resultados, consultas, flechas y focos corregidos permanecen legibles en esas muestras. La captura muestra costo 11, ruta S→B→D→E→T, ocho nodos asentados y nueve mejoras; al final se ve el control de pasos 43/43. El desplazamiento final recorta deliberadamente la parte superior del grafo para mostrar los controles inferiores.
- También se revisó el póster 1920×1080: muestra la misma consulta y ruta de costo 11. Su hash se registra por separado en `visual_review.json`; el póster no se cuenta como fotograma decodificado ni como otra prueba de reproducción.

SHA-256 del MP4 revisado: `93a64bf31516a2b1027ab91555b459de0afb0ef9aa230d957c0efbf4e38c17eb`.
