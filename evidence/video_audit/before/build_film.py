...
def scenes():
    return [
      ...
      {'id':'oracle','title':'Comprobar desde otro algoritmo.','label':'VERIFICACIÓN INDEPENDIENTE','minimum':22,
       'narration':'Para revisar Dijkstra usamos Bellman Ford, un algoritmo independiente, con costos enteros en micro unidades. Contrastamos resultados, conexiones de la ruta y trazas. Se conservan las pruebas que fallaron antes de corregirlas. Esa evidencia muestra lo que se comprobó, sin convertir un conjunto de pruebas en una garantía absoluta.'},
...
async def voice_assets(items):
    ...
    async def one(index,scene):
        audio=WORK/f'voice_{index:02}.mp3'
        metadata=WORK/f'voice_{index:02}.json'
...
class Renderer:
    ...
    def graph(self,draw,example,rect,progress=1.,state=None,route=False,weights=True):
        ...
        for index,edge in enumerate(graph['edges']):
            ...
            if weights and amount>.85:
                ...
                text(draw,label,x,y,size,color if edge['id'] in path_edges else INK,anchor='mm')
    ...
    def frame(self,index,t,duration):
        ...
        if kind=='intro':
            ...
            text(draw,'Texto JSON → validación → Decimal → resultado exacto',90,838,32,TEAL,True)
        elif kind=='cases':
            ...
            for i,(key,title,cost) in enumerate(rows):
                ...
                text(draw,'costo mínimo' if key!='aislado' else 'resultado explícito',x+28,845,24,MUTED)
        elif kind=='oracle':
            ...
            text(draw,'11',120,701,71,CORAL,True);text(draw,'11',1110,701,71,LIME,True)
            text(draw,'=',912,530,85,CORAL,True)
            text(draw,'Comparar costo, validez de ruta y eventos',82,863,31,TEAL,True)
        elif kind=='benchmark':
...
def main():
    ...
    for i,scene in enumerate(items):
        voice=WORK/f'voice_{i:02}.mp3';meta=json.loads((WORK/f'voice_{i:02}.json').read_text(encoding='utf8'))
    ...
    renderer.frame(0,items[0]['duration']*.75,items[0]['duration']).save(OUT/'VerticeSDV_Poster.jpg',quality=94)
    ...
    inputs=[];filters=[]
    for i,scene in enumerate(items):
        inputs+=['-i',str(WORK/f'voice_{i:02}.mp3')]
